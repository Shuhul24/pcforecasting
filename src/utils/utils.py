...
def range_projection(
    ...
    Args:
      current_vertex: torch.Tensor of shape [N, 3] or [N, 4] (x,y,z[,intensity])
    
    Returns:
      proj_range: [H, W] projected range image with depth
      proj_vertex: [H, W, 4] each pixel contains (x,y,z,1)
      proj_idx: [H, W] indices of the raw point cloud
    """
    device = current_vertex.device
    
    # laser parameters (in radians)
    fov_up = torch.tensor(fov_up, dtype=torch.float32, device=device) / 180.0 * torch.pi
    fov_down = torch.tensor(fov_down, dtype=torch.float32, device=device) / 180.0 * torch.pi
    fov = torch.abs(fov_down) + torch.abs(fov_up)
    
    # get depth of all points
    depth = torch.norm(current_vertex[:, :3], dim=1, p=2)
    ...
    proj_x = 0.5 * (yaw / torch.pi + 1.0)  # [0,1]
    proj_y = 1.0 - (pitch + torch.abs(fov_down)) / fov  # [0,1]
    ...
    # round and clamp
    proj_x = torch.clamp(torch.floor(proj_x), 0, proj_W - 1).long()
    proj_y = torch.clamp(torch.floor(proj_y), 0, proj_H - 1).long()
    ...
    scan_y = scan_y[order]
    scan_z = scan_z[order]
    indices = torch.arange(depth.shape[0], device=device)[order]
    
    # initialize projection images
    proj_range = torch.full((proj_H, proj_W), -1.0, device=device)
    proj_vertex = torch.full((proj_H, proj_W, 4), -1.0, device=device)
    proj_idx = torch.full((proj_H, proj_W), -1, device=device, dtype=torch.long)
    
    # fill in images
    proj_range[proj_y, proj_x] = depth
    proj_vertex[proj_y, proj_x, :] = torch.stack(
        [scan_x, scan_y, scan_z, torch.ones_like(scan_x)], dim=1
    )
    proj_idx[proj_y, proj_x] = indices
