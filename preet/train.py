...
def train(cfg,save_dir, dataloader, noise_scheduler, model, optimizer, lr_scheduler, start_epochs, num_epochs, use_wandb=True,visualize=True):
    ...
    for epoch_idx in range(0, start_epochs + num_epochs):
        ...
        # for step in range(1000):
            ...
            naction=target_mask*naction
            # norm_naction = (naction - mean) / std
            # clean_target=norm_naction_full * target_mask
            # breakpoint()
            ...
            # naction = torch.where(naction_gt == -1.0, torch.full_like(naction, -1.0), naction)

            loss_delta_unnorm = loss_L1(target_mask*sample_pred,naction)
            # loss_mask=loss_bce(mask,target_mask)
            # loss_pred=loss_delta+loss_mask
            ...
            # final=masked_rv
            loss_pred = loss_delta_unnorm 
            # if torch.isnan(mask).any() or torch.isinf(mask).any():
            #     print("NaN or Iavgnf found in MASK tensor!")
        ...
        if epoch_idx % 5 == 1 :
            ...
            save_dir='/home/shuhul/ppmf/preet/checkpoints'
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(os.path.join('/home/shuhul/ppmf/preet', "runs"))
    ...
    wandb.init(config=cfg, project='diffusion_model',
                    name='diff_model', dir='/home/shuhul/ppmf/preet')
    ...
    # stat_dict=torch.load('/home/soham/garments/preet/here/PPMFNet/checkpoints/model_epoch_1.pth')
    # contact_model.load(state_dict=stat_dict)
    contact_model = torch.compile(contact_model)
    # contact_model.load_state_dict(stat_dict)
    save_dir='/home/shuhul/ppmf/preet/save'
    # if cfg["num_epochs"] != 0:
