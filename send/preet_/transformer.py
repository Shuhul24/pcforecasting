...
class ContactTransformer(nn.Module):
    def __init__(self, 
        ...
        self.frame_encoder = FrameEncoder(patch_size=16, emb_dim=1024)
    ...
    def forward(self, sample, timestep, obj_feat=None):
        ...
        # tokens, ps = self.frame_encoder(obj_feat)  # (B, F, T, D)
        # B, F, T, D = tokens.shape
        tokens = obj_feat.reshape(B, F,T* D)
        ...
        tokens_sample = sample.reshape(B, F,T*D)
        ...
        output1 = self.seqTransEncoder(xseq)[1:1 + F]
        output = self.output_process(output1)
        output = output.permute(1, 0, 2)
        output = output.view((B, F, T, D))
        output_mask = self.output_process_mask(output1)
        output_mask = output_mask.permute(1, 0, 2)
        output_mask = output_mask.view((B, F, T, D))
        # Decode range and mask separately
        # pred_img = self.frame_decoder(output, ps).squeeze(2)        # (B, 1, H, W)
        # pred_mask_logits = self.frame_decoder_mask(output, ps).squeeze(2)  # (B, 1, H, W)

        # Apply activations
        pred_range = self.min_range + torch.sigmoid(output) * (self.max_range - self.min_range)
        # pred_mask = torch.sigmoid(pred_mask_logits)  # output mask probabilities in [0,1]

        return pred_range,torch.sigmoid(output_mask)
