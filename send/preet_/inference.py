...
# Your model should be in evaluation mode
                ...
                sample_pred = model(noisy_actions, timesteps, obj_feat=conditional_input)
                final_inter = sample_pred + predicted_range[:]
                final = torch.where(final_inter < 0, minus1, final_inter)

                # for t in tqdm(noise_scheduler.timesteps, desc="Conditional Sampling"):
                #     with torch.no_grad():
                #         # ✅ Run the model only ONCE with your conditional input
                #         noise_pred = model(sample_pred, t, obj_feat=conditional_input)
                        
                #         # ✅ Use the scheduler to compute the previous, less-noisy sample
                #         sample_pred = noise_scheduler.step(noise_pred, t, sample_pred).prev_sample
                        # loss=loss_L1(sample_pred, predicted_range)
                        # sample_pred=sample_pred-guidance_scale*2*sample_pred
# --- 3. FINAL RESULT ---
# `sample_pred` now holds the predicted *normalized* delta.
# You need to de-normalize it and add it to your baseline prediction.
# denormalized_delta = sample_pred * std + mean
# final_prediction = predicted_range + denormalized_delta
                ...
                print(loss_pred)
                mask = Projection.get_target_mask_from_range_view(final)
                loss_mask=loss_L1(target_mask,mask)
                print(loss_mask)
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load('/home/soham/garments/preet/here/PPMFNet/checkpoints/model_epoch_41.pth')
